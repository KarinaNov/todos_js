...
const add_new_todo = (title, text) => {
    ...
    console.log(storage.current_todos)
...
const delete_todo_item = id => {
    ...
    if (!check_id) return console.log("Передайте правильный айди задачи");
...
/**
 * 1. Получаем айди редактируемой задачи из разметки
 * 2. находим нужный объект задачи из массива и сохраняем его в специальном поле в сторадже
 * 3. устанавливаем форму в состояние редактирования
 * 4. при обработке формы мы определяем какое происходит событие т.е.добавляние или редактирование
 * 5. изменение данных в объекте задачи
 * 6. вывести разметку в данные
...
const edit_todo_item = (id, title, text) => {
    if (!title) return console.log("Введите титл задачи.");
